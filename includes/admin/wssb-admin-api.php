...
class WP_SEO_SALES_BOOSTER_ADMIN_API
{
    ...
    public function display_field($data = array(), $post = false, $echo = true)
    {
        ...
        switch ($field['type']) {
            ...
            case 'checkbox_multi':
                ...
                break;
            case 'checkbox_multi_google':
                foreach ($field['options'] as $k => $v) {
                    $checked = false;
                    if (in_array($k, (array) $data)) {
                        $checked = true;
                    }
                    $html .= '<p><label for="' . esc_attr($field['id'] . '_' . $k) . '" class="checkbox_multi"><input type="checkbox" ' . checked($checked, true, false) . ' name="' . esc_attr($option_name) . '[]" value="' . esc_attr($k) . '" id="' . esc_attr($field['id'] . '_' . $k) . '" /> ' . $v['label'] . '</label></p> ';
                    if ($checked == true) {
                        $html .= '<input type="text" name= "' . esc_attr($option_name) . '_' . $v['label_name'] . '" class="" placeholder="Add Your Label" value="' . esc_attr(get_option($option_name . '_' . $v['label_name'])) . '">';
                    } else {
                        $html .= '<input type="text" name= "' . esc_attr($option_name) . '_' . $v['label_name'] . '" class="" placeholder="Add Your Label" value="' . esc_attr(get_option($option_name . '_' . $v['label_name'])) . '" style="display: none;">';
                    }
                }
                break;
            case 'checkbox_multi_twitter':
                foreach ($field['options'] as $k => $v) {
                    $checked = false;
                    if (in_array($k, (array) $data)) {
                        $checked = true;
                    ...
                    $html .= '<p><label for="' . esc_attr($field['id'] . '_' . $k) . '" class="checkbox_multi"><input type="checkbox" ' . checked($checked, true, false) . ' name="' . esc_attr($option_name) . '[]" value="' . esc_attr($k) . '" id="' . esc_attr($field['id'] . '_' . $k) . '" /> ' . $v['label'] . '</label></p> ';
                    if ($checked == true) {
                        $html .= '<input type="text" name= "' . esc_attr($option_name) . '_' . $v['label_name'] . '" class="" placeholder="' . $v['placeholder'] . '" value="' . esc_attr(get_option($option_name . '_' . $v['label_name'])) . '">';
                    } else {
                        $html .= '<input type="text" name= "' . esc_attr($option_name) . '_' . $v['label_name'] . '" class="" placeholder="' . $v['placeholder'] . '" value="' . esc_attr(get_option($option_name . '_' . $v['label_name'])) . '" style="display: none;">';
                    }
                }
                break;
            case 'radio_keyword_planner':
                foreach ($field['options'] as $k => $v) {
                    $checked = false;
                    if ($k == $data) {
                        $checked = true;
...
?>
            ...
            case 'editor':
                if (is_array(get_option('wp_seo_sales_booster_notification_type')) && in_array('custom_notification', get_option('wp_seo_sales_booster_notification_type'))) {
                ?>
                    <div class="hidden-editor-container">
                        <?php wp_editor($data, $option_name, array(
